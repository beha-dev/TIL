...
- 일반
  ...
  - 설정 정보는 최상위 단계에 위치
    - 추상화 최상위 단계에 둬야 할 기본값 상수나 설정 관련 상수를 고차원 함수에서 저차원 함수를 호출할 때 인수로 넘김
    - 설정 관련 상수는 최상위 단계에 둠
    - 설정 관련 변수는 나머지 코드에 인수로 넘김
  - 추이적 탐색을 피함
    - 디미터의 법칙 : 자신이 직접 사용하는 모듈만 알아야 함
- 자바
  - 긴 import 목록을 피하고 와일드카드를 사용하라
    - 패키지에서 클래스를 둘 이상 사용한다면 와일드카드를 사용해 패키지 전체를 가져옴
    - import package.*;
  - 상수는 상속하지 않음
  - 상수 대 Enum
- 이름
